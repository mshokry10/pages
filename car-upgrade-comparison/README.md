...
Budget filter matches the lower shopping allowance. The separate priced-advert filter requires an actual checked price within the ceiling. Advert values are dated research, not live offers; equipment and stock need dealer confirmation. The minimum boot filter uses the user's approximate 479 L 72-plate Qashqai N-Connecta reference while noting that some Qashqai configurations are quoted up to 504 L. Tesla Model Y passes editorially because independent luggage tests show a very large usable rear load area, but remains excluded from the numeric litre comparison because Tesla's measurement basis is incompatible. Mustang Mach-E also passes editorially: its 402 L rear boot plus roughly 81 L frunk is close in total, though less useful for a single bulky item. The selection intentionally prioritises Tesla, Audi, BMW, Mercedes, Ford and other European brands.
