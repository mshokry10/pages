...
Budget filter matches the lower shopping allowance. The separate priced-advert filter requires an actual checked price within the ceiling. Advert values are dated research, not live offers; equipment and stock need dealer confirmation. The minimum boot filter now uses the 504 L Qashqai reference. Tesla Model Y passes editorially because independent tests show a very large usable rear load area, but remains excluded from the numeric litre comparison because Tesla's measurement basis is incompatible. Mustang Mach-E remains screened because its comparable rear boot is below the benchmark. The selection intentionally prioritises Tesla, Audi, BMW, Mercedes, Ford and other European brands.
